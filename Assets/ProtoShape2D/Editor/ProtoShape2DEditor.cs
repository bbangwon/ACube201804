...
public class PS2DEditor:Editor{
	...
	ProtoShape2D script;
	bool isDragging=false; //If the point is being dragged
	...
	void Awake(){
		...
		if(script.spriteMaterial==null){
			//script.spriteMaterial=AssetDatabase.GetBuiltinExtraResource<Material>("Sprites-Default.mat");
	...
	*/
	...
	void OnSceneGUI(){
		...
		if(Tools.current!=Tool.None){
			...
			if(Event.current.control || (Event.current.command && (Application.platform==RuntimePlatform.OSXEditor || Application.platform==RuntimePlatform.OSXPlayer))){
				...
				//Draw a marker to add new points, but only if no points are being dragged already
				if(!dontDrawAddPoint) DrawAddPointMarker(isDraggingAnything);
	...
	void DrawDeletePoint(int i){
		...
		Handles.DrawWireDisc(script.transform.TransformPoint(script.points[i].position),objectPlane.normal,size);
		Handles.DrawLine(
		...
		if(Vector2.Distance(script.transform.TransformPoint(script.points[i].position),GetMouseWorldPosition())<size){
	...
	void DrawPoint(int i){
		...
		//Doing it during Layour event because all others return wrong IDs for some reason
		if(et==EventType.Layout) script.points[i].controlID=lastControlID;
		...
		if(changed && !isDragging){
			...
			if(!script.points[i].selected) SelectPoint(i,true);
			//If the dragging just started, we save the initial position
			dragOrigin=script.transform.TransformPoint(script.points[i].position);
			isDragging=true;
		}else if(changed && isDragging){
		...
		//Snapping
		if(script.points[i].controlID==GUIUtility.hotControl && isDragging && Event.current.shift){
			if(script.snapType==PS2DSnapType.Points){
				snap.Reset(size);
				for(int j=0;j<script.points.Count;j++){
					if(j==i || script.points[j].selected) continue; //Don't snap to itself or to other selected points
		...
		//Actual dragging
		if(script.points[i].selected && (changed || (isDragging && Event.current.shift))){
			Undo.RecordObject(script,"Move point");
			Vector3 diff=(Vector2)script.transform.InverseTransformPoint(point)-script.points[i].position;
			//Move all selected points
			for(int j=0;j<script.points.Count;j++){
				if(script.points[j].selected) script.points[j].Move(diff,script.type==PS2DType.Bezier?true:false);
			}
			script.UpdateMesh();
		}
		...
		if(script.points[i].selected==true){
			...
			Handles.DrawSolidDisc(script.transform.TransformPoint(script.points[i].position),objectPlane.normal,size*0.75f);
		}
		//React to drag stop
	...
	void DrawAddPointMarker(bool show){
		bool drawn=false;
		if(!show){
		...
		//Draw non-working control to always keep same number of controls
		if(drawn){
			SetCursor(MouseCursor.ArrowPlus);
		}else{
			/*
			Handles.color=Color.white;
			Handles.Button(script.transform.TransformPoint(Vector3.zero),Quaternion.identity,0f,0f,Handles.CircleHandleCap);
			*/
		}
	}
	...
	void DrawGrid(){
		...
		Vector2 start=script.minPoint;
		Vector2 end=script.maxPoint;
		//If it's a world grid, transforem the points to the world coordinates
		if(script.snapType==PS2DSnapType.WorldGrid){
		...
		//If it's a local grid, transform the rounded up grid to world coordinates
